...
assert(/'clear-light': '#f5f8fb'/.test(popupBootJs) && /'slate-night': '#0d1117'/.test(popupBootJs), 'boot 阶段同步晨雾和石青首帧背景');
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--color-scheme', 'light'), '晨雾使用 light color-scheme');
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--bg', '#f5f8fb'), '晨雾背景使用冬雪冷白');
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--bg-sub', '#eaf0f6'), '晨雾次级背景使用冷雾蓝灰');
...
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--border', '#d5dee8'), '晨雾边框使用雪地蓝灰');
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--accent', '#6f8fab'), '晨雾交互强调使用霜蓝');
...
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--border', '#3d444d'), '石青边框使用 Primer dark borderColor-default');
