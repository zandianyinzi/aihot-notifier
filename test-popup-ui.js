...
assert(/watch-rule-main/.test(popupJs), '规则列表展示关键词数量提示');
...
assert(/\.watch-rules-list\s*{[\s\S]*?max-height:[\s\S]*?overflow-y:\s*auto/.test(popupHtml), '特关规则列表可滚动');
...
assert(hasDeclaration(watchRuleCardRule, 'display', /flex/), '添加后的规则使用单行记录布局');
...
const watchRuleMainRule = popupHtml.match(/\.watch-rule-main\s*{([\s\S]*?)}/i)?.[1] || '';
assert(/white-space:\s*nowrap/.test(watchRuleMainRule), '规则主信息默认单行展示，避免高度碎裂');
assert(/font-size:\s*10px/.test(watchRuleMainRule), '关键词数量提示使用更轻的辅助字级');
