...
assert(hasDeclaration(settingsRule, 'transition', /max-height\s+0\.28s\s+ease/i), '设置面板折叠使用平滑收起动画');
assert(hasDeclaration(settingsPanelOpenRule, 'transition', /max-height\s+0\.18s\s+ease-out/i), '设置面板展开使用高度过渡动画');
assert(!readDeclaration(settingsRule, 'position'), '设置面板参与正常文档流布局');
...
assert(/settingsPanel\.addEventListener\('transitionend',[\s\S]*?requestAnimationFrame\(updateHistoryScrollControls\)/.test(popupJs), '设置面板过渡结束后再刷新历史列表边缘按钮');
...
assert(hasDeclaration(itemRule, 'content-visibility', 'auto'), '历史条目启用离屏内容跳过布局');
assert(hasDeclaration(itemRule, 'contain-intrinsic-size', /auto\s+\d+px/), '历史条目为离屏跳过布局提供固有尺寸');
