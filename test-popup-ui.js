...
assert(hasDeclaration(watchRulesListRule, 'display', 'grid'), '特关规则列表使用网格分栏');
assert(/grid-template-columns\s*:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/.test(watchRulesListRule), '特关规则列表使用两列均分');
...
assert(hasDeclaration(watchRuleCardRule, 'display', /flex/), '添加后的规则使用轻量胶囊布局');
