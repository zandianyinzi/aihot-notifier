...
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--bg-hover', '#f6faf6'), '晴野 hover 带轻草绿质感');
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--bg-item-hover', '#f6faf6'), '晴野列表 hover 不再雾灰');
...
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--rail', '#e86f2e'), '晴野未读热度轨道使用暖色');
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--cat-paper', '#3f8a58'), '晴野分类绿色干净有质感');
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--hot', '#e86f2e'), '晴野保留暖色 AI HOT 热度');
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--bg-unread', '#fff8f0'), '晴野未读背景使用极淡暖色提示');
...
assert(/\.item\.unread\s*{[\s\S]*box-shadow:\s*inset 2px 0 0 color-mix\(in srgb, var\(--rail, var\(--accent\)\) 62%, transparent\)/i.test(popupHtml), '未读条目使用可按主题定制的 Hot rail 热度轨道');
assert(/\.item\.watch-item\.unread\s*{[\s\S]*box-shadow:\s*inset 2px 0 0 var\(--rail, var\(--accent\)\)/i.test(popupHtml), '未读特关条目使用更明确的主题 Hot rail');
