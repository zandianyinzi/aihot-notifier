...
const earlyStyle = popupHtml.match(/<style>([\s\S]*?)<\/style>/i)?.[1] || '';
...
assert(/\.header\s*{[^}]*display\s*:\s*flex/i.test(earlyStyle), '早期内联样式先绘制 header，避免首帧纯色空画布');
assert(/\.list\s*{[^}]*flex\s*:\s*1/i.test(earlyStyle), '早期内联样式先撑开列表区域');
assert(/\.skeleton-item\s*{[^}]*padding\s*:\s*14px\s+18px/i.test(earlyStyle), '早期内联样式先绘制骨架尺寸');
assert(/\.skeleton-line\s*{[^}]*background\s*:\s*rgba\(128,\s*128,\s*128,\s*0\.14\)/i.test(earlyStyle), '早期内联样式先绘制可见骨架线');
assert(!/requestAnimationFrame\(\(\)\s*=>\s*requestAnimationFrame/i.test(popupJs), '缓存命中时不额外等待双 RAF，减少空壳首帧停留');
