...
assert(hasDeclaration(htmlStyle, 'background', '#111110'), 'html 根节点背景硬编码暗色，无需等待 CSS 变量解析');
...
assert(hasDeclaration(bodyStyle, 'background', /var\(--bg,\s*#111110\)/), 'body 内联背景直接依赖主题变量并带兜底');
...
assert(hasDeclaration(viewportRule, 'background', '#111110'), 'html/body 早期共享规则硬编码背景色，确保首帧正确');
...
assert(hasDeclaration(themeCssByName['dark'] || '', '--bg', '#111110'), '墨夜背景使用烟金炭黑');
assert(hasDeclaration(themeCssByName['dark'] || '', '--accent', '#bca77b'), '墨夜强调色使用低饱和烟金');
assert(hasDeclaration(themeCssByName['dark'] || '', '--accent-soft', 'rgba(188,167,123,0.12)'), '墨夜强调轻染跟随烟金');
assert(hasDeclaration(themeCssByName['dark'] || '', '--rail', '#c7ae78'), '墨夜 rail 使用烟金信号色');
assert(hasDeclaration(themeCssByName['dark'] || '', '--rail-strong', '#8f7950'), '墨夜 strong rail 使用深烟金');
assert(hasDeclaration(themeCssByName['green-dark'] || '', '--bg', '#101610'), '暗森背景使用苔绿炭黑');
assert(hasDeclaration(themeCssByName['green-dark'] || '', '--border-light', '#263027'), '暗森轻边界使用绿灰');
assert(hasDeclaration(themeCssByName['green-dark'] || '', '--accent', '#a6b88b'), '暗森强调色使用低饱和苔绿');
assert(hasDeclaration(themeCssByName['green-dark'] || '', '--accent-soft', 'rgba(166,184,139,0.12)'), '暗森强调轻染跟随苔绿');
assert(hasDeclaration(themeCssByName['green-dark'] || '', '--rail', '#b3c596'), '暗森 rail 使用苔绿信号色');
assert(hasDeclaration(themeCssByName['green-dark'] || '', '--rail-strong', '#71845d'), '暗森 strong rail 使用深苔绿');
assert(hasDeclaration(themeCssByName['green-dark'] || '', '--state-ok', '#8dbca6'), '暗森成功态与主题强调色轻微分离');
assert(hasDeclaration(themeCssByName['chrome-dark'] || '', '--accent', '#a6b9d1'), '铬墨强调色使用钛银雾蓝');
assert(hasDeclaration(themeCssByName['chrome-dark'] || '', '--accent-soft', 'rgba(166,185,209,0.12)'), '铬墨强调轻染跟随钛银雾蓝');
assert(hasDeclaration(themeCssByName['chrome-dark'] || '', '--rail', '#b8c9dd'), '铬墨 rail 使用雾蓝信号色');
assert(hasDeclaration(themeCssByName['chrome-dark'] || '', '--rail-strong', '#73879e'), '铬墨 strong rail 使用深雾蓝');
assert(hasDeclaration(themeCssByName['chrome-dark'] || '', '--border', '#303640'), '铬墨边框使用钛灰边界');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--color-scheme', 'dark'), '石青使用 dark color-scheme');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--bg', '#0e181b'), '石青背景使用矿物青黑');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--bg-sub', '#162328'), '石青次级背景使用青黑面板色');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--bg-hover', '#203238'), '石青 hover 保持低噪声青黑层级');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--text', '#dce8e7'), '石青主文字使用雾白');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--text-2', '#abbfc0'), '石青次级文字使用灰青');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--border', '#293e44'), '石青边框使用矿物青灰');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--accent', '#86b7af'), '石青交互强调使用灰青绿');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--accent-soft', 'rgba(134,183,175,0.12)'), '石青强调轻染跟随灰青绿');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--rail', '#9ac9bf'), '石青 rail 使用灰青绿信号色');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--rail-strong', '#5b8d88'), '石青 strong rail 使用深灰青绿');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--cat-products', '#8ab3c8'), '石青产品分类色降低硬蓝感');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--cat-paper', '#93b8a1'), '石青论文分类色降低模板绿感');
assert(hasDeclaration(themeCssByName['slate-night'] || '', '--cat-tips', '#c49a94'), '石青观点分类色降低硬红感');
...
assert(/data-action="toggle" title="\$\{toggleAction\}" aria-label="\$\{toggleAction\} \$\{ruleLabel\}"/.test(popupJs), '规则启停按钮保留完整操作提示和规则名称');
...
assert(hasDeclaration(itemReadHoverRule, 'background', /color-mix\(in srgb, var\(--bg\) 86%, #000\)/), '已读条目 hover 使用基于背景的轻微压暗反馈');
