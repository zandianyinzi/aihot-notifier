...
assert(/\.item\.unread\s*{[\s\S]*box-shadow:\s*inset 2px 0 0 var\(--rail, var\(--accent\)\)/i.test(popupHtml), '未读条目默认回退主题主色 Hot rail');
assert(/\.item\.watch-item\.unread\s*{[\s\S]*box-shadow:\s*inset 2px 0 0 var\(--rail-strong, var\(--accent\)\)/i.test(popupHtml), '未读特关条目默认回退主题主色 Hot rail');
