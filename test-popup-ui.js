...
assert(/watchRulesList\.innerHTML\s*=\s*'<div class="watch-rule-text">暂无特关规则<\/div>'/.test(popupJs), '特关空规则描述保持简短');
...
assert(hasDeclaration(watchRuleFormRule, 'border', /1px\s+dashed\s+var\(--border-light\)/), '新增规则表单外框使用轻量虚线');
...
assert(hasDeclaration(watchRuleCardRule, 'border', /1px\s+dashed\s+var\(--border-light\)/), '新增后的规则卡片外框使用轻量虚线');
assert(hasDeclaration(watchRuleCardRule, 'padding', /9px\s+8px/), '规则卡片删除左侧竖条后使用均衡左右内边距');
...
assert(!hasDeclaration(dateLabelRule, 'float', /right|left/), '日期浮标不使用 float 覆盖首条标题');
assert(!hasDeclaration(dateLabelRule, 'margin-bottom', /-\d+px/), '日期浮标不使用负 margin 叠在列表内容上');
assert(hasDeclaration(dateLabelRule, 'margin', /6px\s+18px\s+6px\s+auto/), '日期浮标在正常文档流内靠右排列');
