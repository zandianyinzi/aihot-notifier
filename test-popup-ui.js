...
assert(/<option value="clear-light">晨雾<\/option>/.test(themeSelectHtml), '外观主题下拉包含晨雾，且不使用 GitHub 命名');
...
assert(/const VALID_THEMES = new Set\(\['dark', 'green-dark', 'chrome-dark', 'clear-light', 'slate-night'\]\)/.test(popupJs), 'JS 主题白名单接受晨雾和石青');
assert(/\['dark', 'green-dark', 'chrome-dark', 'clear-light', 'slate-night'\]\.includes\(theme\)/.test(popupBootJs), 'boot 阶段主题白名单接受晨雾和石青');
assert(/'clear-light': '#fbfdff'/.test(popupBootJs) && /'slate-night': '#0d1117'/.test(popupBootJs), 'boot 阶段同步白净晨雾和石青首帧背景');
...
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--color-scheme', 'light'), '晨雾使用 light color-scheme');
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--bg', '#fbfdff'), '晨雾背景使用通透净白');
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--bg-sub', '#ffffff'), '晨雾面板使用纸面白');
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--bg-hover', '#f4f8fc'), '晨雾 hover 使用清透浅白蓝');
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--bg-item-hover', '#f4f8fc'), '晨雾列表 hover 不再雾灰');
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--border-light', '#edf2f7'), '晨雾弱边界更白净');
...
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--border', '#e4ebf3'), '晨雾边框使用干净浅边界');
...
assert(hasDeclaration(themeCssByName['clear-light'] || '', '--bg-unread', '#f4faff'), '晨雾未读背景保持白净轻提示');
