...
assert(hasDeclaration(watchRuleCardRule, 'grid-template-columns', /minmax\(0,\s*1fr\)\s+auto/), '规则卡片使用左内容右操作的两列结构');
assert(!hasDeclaration(watchRuleCardRule, 'grid-template-rows', /auto\s+auto/), '规则卡片不使用无效跨行布局，避免关键词增减时展示不稳定');
...
assert(hasDeclaration(watchRuleContentRule, 'grid-column', '1'), '规则内容区占据左侧主栏');
assert(!hasDeclaration(watchRuleContentRule, 'grid-row', /1\s*\/\s*span\s*2/), '规则内容区不再跨无效网格行');
assert(/display:\s*flex/.test(watchRuleContentRule), '规则内容区仍采用纵向堆叠');
